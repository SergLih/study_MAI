...
NUM_FROM_OTHER_VAR = 
...
class FactorBasePrime:
    def __init__(self, prime, tmem, logprime):
        self.prime = prime        # the factor base prime
        self.soln1 = None
        self.soln2 = None
        self.tmem = tmem
        self.logprime = logprime  #log2(prime)rounded to nearest
        self.ainverse = None
...
def test_miller_rabin(n, s=None):
    if n == 2:
        return True
    ...
    for _ in range(s):
        ...
        j = 0
        z = pow_mod(b, m, n)
        while not ((j == 0 and z == 1) or z == n - 1):
...
def siqs_init_stage_first_poly(n, m, factor_base):
    ...
    for i, fb in enumerate(factor_base):
        if p_min_i is None and fb.prime >= SIQS_MIN_PRIME_POLYNOMIAL:
            p_min_i = i
        if p_max_i is None and fb.prime > SIQS_MAX_PRIME_POLYNOMIAL:
            p_max_i = i - 1
            break
    ...
    target1 = target / ((factor_base[p_min_i].prime +
                         factor_base[p_max_i].prime) / 2) ** 0.5
    ...
    for fb in factor_base:
        if a % fb.prime != 0:
            fb.ainverse = inv_mod(a, fb.prime)
            fb.soln1 = (fb.ainverse * (fb.tmem - b)) % fb.prime
            fb.soln2 = (fb.ainverse * (-fb.tmem - b)) % fb.prime

    return g, h, B

def siqs_init_stage_next_poly(n, factor_base, i, g, B):
...
def siqs_build_matrix(factor_base, smooth_relations):
    ...
    """
    fb = len(factor_base)
    sm = len(smooth_relations)
    M = np.zeros((sm, fb), dtype=np.bool)
    for i in range(sm):
        for j, exp in smooth_relations[i][2]:
            M[i][j] = exp % 2
    return M

def find_pivot_column(M, j): #!
    ...
    if rem != 1 and non_prime_factors:
        ...
        for fact in sorted(siqs_find_more_factors_gcd(non_prime_factors)):
            while fact != 1 and rem % fact == 0:
                ...
                factors.append(fact)
                rem //= fact
            if rem == 1 or test_miller_rabin(rem):
                break
...
def siqs_factorise(n):
    ...
    while not success:
        ...
        while not enough_relations:
            if i_poly == 0:
                g, h, B = siqs_init_stage_first_poly(n, m, factor_base)
            else:
                g, h = siqs_init_stage_next_poly(n, factor_base, i_poly, g, B)
            i_poly += 1
            if i_poly >= 2 ** (len(B) - 1):
                i_poly = 0
            ...
            if (len(smooth_relations) >= required_relations or
                ...
                print("\rTotal %d/%d relations." %
                      (len(smooth_relations), required_relations), end='')
        ...
        perfect_squares = siqs_solve_matrix_opt(M)
...
def factorise(n):
    ...
    if test_miller_rabin(n):
        return [n]
    factors, rem = trial_div_init_primes(n, 100000000)
    if factors:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('usage: python quadratic_sieve.py <input.txt>')
        sys.exit(1)
    ...
    if type(n1) != int or n1 < 1:
        print("\nNumber needs to be an integer >= 1")
    else:
        print("\nSuccess. Prime factors: %s" % factorise(n1))
    print("\nTime:", time() - t_start)

    #Факторизация второго числа
    factors = []
    factors.append(gcd(n2, NUM_FROM_OTHER_VAR))
    factors.append(n2 // factors[0])
