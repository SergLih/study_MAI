...
class BiCG:
    def __init__(self, slau, eps=EPS, max_iter = 2000):
        self.matrix = sp.csc_matrix(slau[0])
        self.b = slau[1]
        self.eps = eps
...
with open(outfilename, 'wt') as f_out:
    print("Order matrix: ", len(matrix), file=f_out)
    print("Source matrix:\n", mat2[:, :-1].round(1), file=f_out)
    print("Answer's vector:\n", mat2[:, -1].round(1), file=f_out)
    print("Precision: ", EPS, file=f_out)

start0 = time()   
solver = BiCG(slau, max_iter=100000)
x0 = solver.bicg()
end0 = time()
...
start3 = time()
slau = mat2[:, :-1], mat2[:, -1]
x3 = np.linalg.solve(slau[0], slau[1])
end3 = time()
with open(outfilename, 'at') as f_out:        
