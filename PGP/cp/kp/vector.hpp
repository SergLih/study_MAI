...
template <typename T> struct vec<3,T> {
    ...
    vec<3,T> normalize(T l=1) const { return (*this)*(l/norm()); }
    T x,y,z;
};
...
template <typename T> vec<3,T> operator!(const vec<3,T>  &v) {
    return vec<3,T> (v.y, v.z, v.x);
}
...
template <typename T> bool allclose(vec<3,T> v1, vec<3,T> v2) {
