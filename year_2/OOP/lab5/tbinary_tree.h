...
template<class T>
class TreeNode : std::enable_shared_from_this<TreeNode<T> >{
public:
	TreeNode();
	TreeNode(std::shared_ptr<T> data, TreeNodePtr<T> parent = nullptr);
	TreeNodePtr<T> GetNext();
...
template<class T>
inline TBinaryTreeIterator<T> TBinaryTree<T>::begin()
{
	return TBinaryTreeIterator<T>(root);
}

template<class T>
inline TBinaryTreeIterator<T> TBinaryTree<T>::end()
{
	return nullptr;
}
...
template<class T>
TreeNode<T>::TreeNode(std::shared_ptr<T> data, TreeNodePtr<T> parent = nullptr)
{
	left = nullptr;
	right = nullptr;
	this->parent = parent;
