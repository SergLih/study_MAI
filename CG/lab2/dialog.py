...
def SaveFile(event):
	...
	if not fn.endswith(".txt"):
		fn+=".txt"
	open(fn, 'w').write(inp_a.get() + ' '+ inp_h.get() + ' ' + inp_color.get() + ' ' + inp_view.get())

#функция построения призмы
def Build(event):
	plot_prism(float(inp_a.get()), float(inp_h.get()), int(inp_color.get()), float(inp_view.get()))
...
man = Label(text = "Введите высоту, длину стороны, цвет и степень прозрачности.", font = "Arial 10")
man.pack(side = "top")
